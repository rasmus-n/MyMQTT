...
{
  public:
    hand(const char* topic, void (*handler)(const char* topic, uint8_t* payload, unsigned int length))
    {
  ...
  private:
    void (*m_handler)(const char* topic, uint8_t* payload, unsigned int length) = NULL;
